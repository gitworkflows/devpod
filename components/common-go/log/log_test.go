...
func TestScrubFormatter(t *testing.T) {
	...
	logger.WithTime(logTime).WithField("info", info).WithField("workspaceID", workspaceID).
		...
		Info("email: anton@devpod.khulnasoft.com")

	expectation := fmt.Sprintf(
		`{"error":"some test error","info":"[redacted:nested]","level":"info","message":"email: anton@devpod.khulnasoft.com","serviceContext":{"service":"test","version":"1.0.0"},"severity":"INFO","time":"%s","workspaceID":"[redacted:md5:e807f1fcf82d132f9bb018ca6738a19f]"}`,
...
func BenchmarkSimpleScrubFormatter(b *testing.B) {
	...
	run := func() {
		logger.WithField("workspaceID", "1234567890").Info("email: anton@devpod.khulnasoft.com")
	}
